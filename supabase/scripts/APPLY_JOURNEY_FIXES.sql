...
BEGIN
  IF NOT EXISTS (
    ...
    UPDATE weekly_summaries SET created_at = period_start::timestamptz WHERE created_at IS NULL AND period_start IS NOT NULL;
  END IF;
END $$;
...
CREATE POLICY "Users can view own summaries" ON weekly_summaries FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own summaries" ON weekly_summaries;
CREATE POLICY "Users can insert own summaries" ON weekly_summaries FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own summaries" ON weekly_summaries;
CREATE POLICY "Users can update own summaries" ON weekly_summaries FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can view own responses" ON question_responses FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own responses" ON question_responses;
CREATE POLICY "Users can insert own responses" ON question_responses FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own responses" ON question_responses;
CREATE POLICY "Users can update own responses" ON question_responses FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can view own corpora" ON file_search_corpora FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own corpora" ON file_search_corpora;
CREATE POLICY "Users can insert own corpora" ON file_search_corpora FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own corpora" ON file_search_corpora;
CREATE POLICY "Users can update own corpora" ON file_search_corpora FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
