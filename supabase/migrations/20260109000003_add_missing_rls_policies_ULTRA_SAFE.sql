-- ============================================================================
-- Migration: Add Missing RLS Policies for Unified Timeline Tables (ULTRA SAFE)
-- Date: 2026-01-09
-- Priority: CRITICAL - Security gaps identified in audit
-- Note: Verifies both table AND column existence before applying policies
-- ============================================================================
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'daily_questions') THEN
    ...
    IF has_created_by_ai THEN
      -- CREATE INSERT policy with created_by_ai check
      EXECUTE '
      ...
      ';
      RAISE NOTICE '✅ daily_questions: Added INSERT policy with created_by_ai check';
    ELSE
      -- CREATE INSERT policy without created_by_ai check
      EXECUTE '
        CREATE POLICY "Service role can insert questions"
          ...
          WITH CHECK (true)
      ';
      RAISE NOTICE '✅ daily_questions: Added INSERT policy without created_by_ai check';
      RAISE WARNING '⚠️ daily_questions: created_by_ai column not found - policy allows all service_role inserts';
    END IF;
    ...
    RAISE NOTICE '✅ daily_questions: Added INSERT policies';
  ELSE
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'question_responses') THEN
    ...
    ELSE
      RAISE WARNING '⚠️ question_responses: Expected at least 2 policies, found %', policy_count;
    END IF;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'weekly_summaries') THEN
    ...
    ELSE
      RAISE WARNING '⚠️ weekly_summaries: Expected at least 2 policies, found %', policy_count;
    END IF;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'whatsapp_user_activity') THEN
    ...
    ELSE
      RAISE WARNING '⚠️ whatsapp_user_activity: Expected at least 1 policy, found %', policy_count;
    END IF;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'daily_questions') THEN
    ...
    ELSE
      RAISE WARNING '⚠️ daily_questions: Expected at least 2 policies, found %', policy_count;
    END IF;
  END IF;

  -- Verify user_activities
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'user_activities') THEN
    SELECT COUNT(*) INTO policy_count FROM pg_policies WHERE tablename = 'user_activities';
    IF policy_count >= 3 THEN
      RAISE NOTICE '✅ user_activities: % policies verified', policy_count;
    ELSE
      RAISE WARNING '⚠️ user_activities: Expected at least 3 policies, found %', policy_count;
    END IF;
  END IF;
