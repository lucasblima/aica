...
import { useAssociationsData } from './src/hooks/useAssociationsData';
...
export default function App() {
   ...
   const StackedBarChart = () => {
      const data = workloadDistribution;
   ...
   const PriorityDonut = () => {
      const data = priorityDistribution;
            ...
            <section className="grid grid-cols-2 gap-3">
               ...
               <div className="glass-panel p-4 rounded-2xl bg-white border border-slate-100 shadow-sm relative overflow-hidden group">
                  <div className="flex justify-between items-start mb-2">
                     ...
                     <div className="flex items-center text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">
                        <ArrowUpRight className="w-3 h-3 mr-0.5" /> {kpi.assocTrend}%
                     </div>
                  </div>
                  <div className="relative z-10">
                     <span className="text-3xl font-black text-slate-800 block">{kpi.totalAssociations}</span>
               ...
               <div className="glass-panel p-4 rounded-2xl bg-white border border-slate-100 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                     ...
                     <div className="flex items-center text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">
                        <ArrowUpRight className="w-3 h-3 mr-0.5" /> {kpi.activeUsersTrend}%
                     </div>
                  </div>
                  <div>
                     <span className="text-3xl font-black text-slate-800 block">{kpi.activeUsers}</span>
                     <span className="text-[10px] text-slate-500 font-medium">Usuários Conectados</span>
                  </div>
               </div>

               {/* Card: Sync Health */}
               <div className={`col-span-1 p-4 rounded-2xl border shadow-sm flex flex-col justify-between ${systemHealth.failedSyncs > 0 ? 'bg-rose-50 border-rose-100' : 'bg-white border-slate-100'}`}>
                  <div className="flex justify-between items-start">
                     <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${systemHealth.failedSyncs > 0 ? 'bg-white text-rose-500' : 'bg-emerald-50 text-emerald-600'}`}>
                        <Activity className="w-4 h-4" />
                     </div>
                     {systemHealth.failedSyncs > 0 && <span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span>}
                  </div>
                  <div>
                     <div className="flex items-baseline gap-1">
                        <span className={`text-2xl font-black ${systemHealth.failedSyncs > 0 ? 'text-rose-700' : 'text-slate-800'}`}>
                           {systemHealth.syncSuccessRate}%
                        </span>
                     </div>
                     <span className={`text-[10px] font-medium ${systemHealth.failedSyncs > 0 ? 'text-rose-600' : 'text-slate-500'}`}>
                        {systemHealth.failedSyncs > 0 ? `${systemHealth.failedSyncs} Falhas Críticas` : 'Sincronização OK'}
                     </span>
                  </div>
               </div>
