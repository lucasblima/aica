-- ============================================================================
-- HOTFIX: Create ai_usage_analytics table in Staging
-- ============================================================================
-- ISSUE: Migration 20251208180300_multimodal_analytics.sql não foi aplicada
-- SOLUTION: Execute este script no Supabase SQL Editor (staging)
-- ============================================================================

-- Step 1: Verificar se tabela já existe
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables
    WHERE table_schema = 'public'
    AND table_name = 'ai_usage_analytics'
  ) THEN
    RAISE NOTICE '✅ Tabela ai_usage_analytics já existe';
  ELSE
    RAISE NOTICE '⚠️ Tabela ai_usage_analytics NÃO existe - será criada';
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.ai_usage_analytics (
  ...
  module_id UUID,
  asset_id UUID,  -- Removido FK por enquanto (ai_generated_assets pode não existir)
...
BEGIN
  ...
  IF v_table_exists THEN
    RAISE NOTICE '✅ Table ai_usage_analytics: EXISTS';
  ELSE
    RAISE NOTICE '❌ Table ai_usage_analytics: MISSING';
  END IF;

  RAISE NOTICE '✅ RLS Policies: % found (expected: 2)', v_policy_count;
  RAISE NOTICE '✅ Indexes: % found (expected: 6)', v_index_count;
  RAISE NOTICE '================================================';
  RAISE NOTICE '';

  IF v_table_exists AND v_policy_count >= 2 AND v_index_count >= 6 THEN
    RAISE NOTICE '🎉 SUCCESS! Table created and configured correctly!';
  ELSE
...
1. ✅ Re-run the Quick Health Check:
   SELECT COUNT(*) as total_records FROM ai_usage_analytics;

2. ✅ Test AI operations in staging:
   - Journey: Create a moment
   - Grants: Generate field content
   - Finance: Parse a statement
