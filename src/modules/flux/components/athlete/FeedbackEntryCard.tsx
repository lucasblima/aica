 ...
 * summary, and workout count. Inspired by Journey CeramicMomentCard.
 */

import { motion } from 'framer-motion';
import { Calendar, Dumbbell, MessageSquare } from 'lucide-react';
import type { FeedbackEntry } from '../../hooks/useAthleteFeedback';
...
export function FeedbackEntryCard({ entry, index = 0 }: FeedbackEntryCardProps) {
      ...
      <div className="space-y-1">
        {entry.slots.map((slot) => (
          <div key={slot.slotId} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-ceramic-text-secondary w-8">{DAY_NAMES_SHORT[slot.dayOfWeek] || ''}</span>
