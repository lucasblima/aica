...
import { suggestPriority, QUADRANT_MAP, type PrioritySuggestion } from '@/services/taskAIService';
      ...
      <AnimatePresence>
        {autoCreatedTitle && (
          <motion.div
            ...
            className="flex items-center gap-2.5 p-3 mb-3 bg-ceramic-success/10 border border-ceramic-success/20 rounded-xl"
          >
            <CheckCircle2 className="w-5 h-5 text-ceramic-success shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm text-ceramic-success font-bold truncate">{autoCreatedTitle}</p>
              {autoCreatedDetails && (
                <p className="text-xs text-ceramic-success/80 mt-0.5">Agendada para {autoCreatedDetails}</p>
              )}
            </div>
            <span className="text-[10px] text-ceramic-success/70 font-medium uppercase tracking-wide shrink-0">Criada</span>
          </motion.div>
        )}
      </AnimatePresence>
                    ...
                    {(['transcribing', 'extracting'] as const).map((stage, idx) => {
                      ...
                      return (
                        <React.Fragment key={stage}>
                          {idx > 0 && <div className={`h-px flex-1 max-w-[20px] transition-colors duration-300 ${isDone ? 'bg-ceramic-success' : 'bg-ceramic-text-secondary/20'}`} />}
                          <div className={`flex items-center gap-1.5 transition-all duration-300 ${isActive ? 'opacity-100 scale-100' : isDone ? 'opacity-60 scale-95' : 'opacity-30 scale-95'}`}>
                            {isActive ? <Loader2 className={`w-4 h-4 animate-spin ${config.color}`} /> : isDone ? <CheckCircle2 className="w-4 h-4 text-ceramic-success" /> : <Icon className="w-4 h-4 text-ceramic-text-secondary/40" />}
                            <span className={`text-[11px] font-medium whitespace-nowrap ${isActive ? config.color : isDone ? 'text-ceramic-success' : 'text-ceramic-text-secondary/40'}`}>
                ...
                <div className="space-y-2">
                  ...
                  {(previewDueDate || previewScheduledTime) && (
                    <div className="flex items-center gap-2 flex-wrap">
                      {previewDueDate && (
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-ceramic-info/10 text-ceramic-info rounded-lg text-xs font-medium">
                          <Calendar className="w-3 h-3" />
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="list-content" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-2 overflow-hidden">
                  {previewChecklistItems.map((item, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 ceramic-inset p-2.5 rounded-lg group">
                      ...
                      <button type="button" onClick={() => setPreviewChecklistItems(prev => prev.filter((_, i) => i !== idx))} className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-ceramic-text-secondary hover:text-ceramic-error">
              ...
              {previewTaskType === 'event' && (
                <motion.div key="event-content" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-3 overflow-hidden">
                  ...
                  {isCalendarConnected && (
                    <div className="flex items-center gap-1.5 text-xs text-ceramic-success font-medium">
